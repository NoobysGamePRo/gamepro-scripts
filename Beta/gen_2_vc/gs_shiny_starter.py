"""
Gold / Silver Shiny Starter
Game: Pokemon Gold / Silver (3DS Virtual Console)
...
Ported from VC_GS_Shiny_Starter_4.0.cpp.

Setup:
  - Save inside Prof. Elm's lab, standing in front of the Poke Ball case.
  - On first run the script navigates through the receive sequence and
    asks you to draw a region over each starter's sprite in turn.
  - Calibration is saved so subsequent runs skip the setup step.
  - Delete calibration/vc_gs_shiny_starter.json to recalibrate.

Timing notes:
  - All delays are tunable via the constants at the top of the class.
  - Gold/Silver and Crystal share the same starter reception sequence,
    so timing is nearly identical. Adjust if your game behaves differently.
"""
...
def _cal_path() -> str:
    ...
    return os.path.join(cal_dir, 'vc_gs_shiny_starter.json')


class GSShinyStarter(BaseScript):
    NAME = "VC Gold/Silver – Shiny Starter"
    DESCRIPTION = (
        "Soft-resets Pokemon Gold/Silver (3DS VC) for a shiny starter. "
    ...
    RECEIVE_DELAY      = 1.3   # between A presses through receiving text
    ...
    ELM_DIALOGUE_DELAY = 1.3   # between A presses through Elm dialogue
    ...
    GOOD_LUCK_WAIT     = 2.8   # wait before final dialogue
    GOOD_LUCK_DELAY    = 1.5   # after final A press
    ...
    def run(self, controller, frame_grabber, stop_event, log, request_calibration):
        log("VC Gold/Silver Shiny Starter started.")
        ...
        while not stop_event.is_set():
            ...
            for _ in range(4):   # Elm dialogue
                if stop_event.is_set(): break
                controller.press_a()
                if not self.wait(self.ELM_DIALOGUE_DELAY, stop_event): break
            if stop_event.is_set(): break
        ...
        log("VC Gold/Silver Shiny Starter stopped.")
    ...
    def _calibrate(self, controller, frame_grabber, stop_event,
        ...
        log("Edit calibration/vc_gs_shiny_starter.json to change 'tolerance'.")
