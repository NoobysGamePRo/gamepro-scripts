...
Soft-resets for shiny Darkrai on Newmoon Island (requires Member
Card event item). Darkrai is a fixed-position encounter.

Ported from Shiny_Darkrai_1.0.cpp.
...
How it works:
  ...
  4. Presses A to interact with Darkrai; waits for battle blackout.
  5. Waits BATTLE_LOAD_WAIT s for the Darkrai sprite to appear.
  6. avg_rgb check on the calibrated region vs. baseline ± tolerance.

Setup:
  - Save directly in front of Darkrai on Newmoon Island.
  - On first run let the battle load and draw a region over Darkrai's
    battle sprite.
  - Delete calibration/bdsp_shiny_darkrai.json to recalibrate.
"""
...
def _cal_path() -> str:
    ...
    return os.path.join(cal_dir, 'bdsp_shiny_darkrai.json')


class BDSPShinyDarkrai(BaseScript):
    NAME = "BDSP - Shiny Darkrai"
    DESCRIPTION = "Soft-resets for shiny Darkrai (Brilliant Diamond/Pearl)."
    ...
    def run(self, controller, frame_grabber, stop_event, log, request_calibration):
        log("BDSP - Shiny Darkrai started.")

        cal = self._load_calibration()
        if cal is None:
            log("No calibration found — starting first-run setup.")
            log("Let the Darkrai battle load, then draw a region over its sprite.")
            cal = self._calibrate(
        ...
        log("BDSP - Shiny Darkrai stopped.")
