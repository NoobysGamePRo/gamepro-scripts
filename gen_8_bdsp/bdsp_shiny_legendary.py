"""
BDSP - Shiny Legendary
Game: Pokemon Brilliant Diamond / Shining Pearl (Nintendo Switch)

Soft-resets for miscellaneous shiny legendaries accessed via a
static encounter spot (e.g. Dialga/Palkia at Spear Pillar, Giratina
at Turnback Cave, and other fixed legendaries).

Ported from BDSP_Shiny_Box_Legendary_1.1.cpp.

How it works:
  The C++ version uses white-pixel counting (PNI) to detect the
  battle text appearing, then waits for consecutive white pixels to
  disappear before checking for the battle box. The Python port uses
  the simpler avg_rgb comparison approach used throughout the project.

  1. Soft-resets (S command via Home button on Switch) and waits
     SOFT_RESET_WAIT s.
  2. A × NUM_MENU_A with MENU_DELAY s between each (title/continue).
  3. Waits OVERWORLD_WAIT s for the world to load.
  4. Presses A to interact with the legendary spot; waits for
     battle blackout.
  5. Waits BATTLE_LOAD_WAIT s for the legendary sprite to appear.
  6. avg_rgb check on the calibrated region vs. baseline ± tolerance.

Setup:
  - Save directly in front of the legendary encounter spot.
  - On first run let the battle load and draw a region over the
    legendary's battle sprite.
  - Delete calibration/bdsp_shiny_legendary.json to recalibrate.
"""
...
def _cal_path() -> str:
    ...
    return os.path.join(cal_dir, 'bdsp_shiny_legendary.json')


class BDSPShinyLegendary(BaseScript):
    NAME = "BDSP - Shiny Legendary"
    DESCRIPTION = "Soft-resets for miscellaneous shiny legendaries (Brilliant Diamond/Pearl)."

    # ── Timing (seconds) ─────────────────────────────────────────────────────
    SOFT_RESET_WAIT    = 25.0   # Switch game reload (BDSP is slower)
    ...
    def run(self, controller, frame_grabber, stop_event, log, request_calibration):
        log("BDSP - Shiny Legendary started.")
        ...
        log("BDSP - Shiny Legendary stopped.")
